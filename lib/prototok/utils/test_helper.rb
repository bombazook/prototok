    ...
    module TestHelper
      def option_combinations
        combinations = (versions+[nil]).map do |v|
          [v].product(cipher_names(v)).product(encoder_names).product(formatter_names)
      ...
      def prepare_options o={}
        ...
        if options[:encoder].to_s == 'protobuf'
          ...
          payload_proto_path = File.join(Prototok::Utils::Paths.gem_root, 'spec/encoders/protobuf/test_payload.prot')
      ...
      def itemize_options keyword_args={}
        defaulted = keyword_args.inject({}) do |aggr, (key, val)|
          aggr[key] = (val.nil? ? 'default' : val)
          aggr
        end
        name = defaulted.to_a.map{|i| i.join(' is ')}.join(', ')
        {name => keyword_args.select{|k, v| !v.nil? }}
      end
