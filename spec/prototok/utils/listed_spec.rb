...
RSpec.describe Prototok::Utils::Listed do
  ...
  let(:seeking_const) do
    extended_class::SomeConst = 'value'.freeze
    extended_class::SomeConst
  end
  let(:namespace_module) do
    extended_class::Namespace = Module.new
  end
  let(:namespaced_const) do
    namespace_module::SomeConst = 'namespaced_value'.freeze
  end
  ...
  context '1 argument' do
    it 'tries to get camelized const from class' do
      expect(extended_class)
        .to receive(:const_get).with('NonExistantConst', false)
        .and_call_original
  ...
  context '2+ arguments' do
    it 'tries to get camelized const from class' do
      expect(extended_class)
        .to receive(:const_get).with('Namespace::SomeConst', false)
        .and_call_original
      extended_class.find 'namespace', 'some_const'
    end

    it 'returns const if it exists' do
      expect(namespaced_const)
        .to be_eql(extended_class.find('namespace', 'some_const'))
    end

    it 'returns nil if it doesnt exist' do
      expect(nil)
        .to be_eql(extended_class.find('other_namespace', 'some_other_const'))
    end
  end
end
